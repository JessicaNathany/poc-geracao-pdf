...
namespace gerador_pdf.api.Service
{
    ...
    {
        public List<Pessoa> ObtendoArquivoJsonESerializandoEmUmObjeto(string parametro)
        {
            var caminhoArquivo = string.Empty;

            if (parametro == "pessoas")
            {
                caminhoArquivo = @"C:\Github\pessoas.json";
            }
            else
            {
                caminhoArquivo = @"C:\Github\relatorioRoteiroGravacao.json";
            }
        ...
        public void CriarCelulaTextoPessoas(PdfCelulaTextoModel celulaTexto)
        {
